...
from .html_generator import create_volume_page, create_chapter_page, create_section_page, create_chapter, create_toc_page
...
def _extract_copyright_metadata(content: str) -> Dict[str, str]:
    """Extract EPUB metadata from a Korean ``판권`` block in the source TXT."""
    match = re.search(r'(?:^|\n)\s*판권\s*\n(?P<body>[\s\S]*?)\s*$', content or '')
    if not match:
        return {}
    ...
    for line in body.splitlines():
        ...
        if not field:
            continue
        key = label_map.get(field.group(1).strip())
...
def _resolve_book_metadata(
    ...
    if resolved_language not in {'ko', 'zh', 'en'}:
        resolved_language = {
            'korean': 'ko',
            'chinese': 'zh',
            'english': 'en',
    ...
    rights = overrides.get('rights') or generated.get('rights') or ''
    ...
    metadata_payload = {
        ...
        'subjects': subjects,
        'copyright_page': bool(overrides.get('copyright_page', False)),
        'copyright_content': str(overrides.get('copyright_content') or '').strip(),
    }
...
def _has_copyright_chapter(volumes: List[Volume]) -> bool:
    """Return True when source text already has a dedicated copyright chapter."""
...
def _build_copyright_page_content(title: str, author: str, metadata: Dict[str, Any]) -> str:
    """Build plain-text copyright page content from available book metadata."""
    lines = []
    if title:
        lines.append(f"작품명: {title}")
    if author:
        lines.append(f"저자: {author}")
    if metadata.get("publisher"):
        lines.append(f"발행처: {metadata['publisher']}")
    if metadata.get("date"):
        lines.append(f"발행일: {metadata['date']}")
...
def _read_txt_file(txt_file: str) -> str:
    ...
    try:
        ...
        with open(txt_file, 'rb') as f:
            # A small number of malformed trailing bytes can make chardet reject
            # an otherwise valid Korean file. A representative prefix is more
            # reliable than feeding it the entire document.
            sample_size = min(file_size, 100_000)
        ...
        encodings_to_try = [encoding, 'utf-8', 'cp949', 'euc-kr', 'gb18030', 'gbk', 'utf-16', 'latin1']
            ...
            try:
                with open(txt_file, 'r', encoding=enc, errors='replace') as f:
                    ...
                    if content and content.count('�') / len(content) < 0.1:  # Less than 10% replacement characters
                        # Some legacy Korean TXT files contain a UTF-8 copyright
                        # sign inside otherwise EUC-KR/CP949 text.
                        content = re.sub(r'�(?=\s*\d{4}\b)', '©', content)
                        return content
...
def txt_to_epub(txt_file: str, epub_file: str, title: Optional[str] = None,
    ...
    try:
        with tqdm(total=5, desc="Conversion Progress", disable=not show_progress, ncols=80) as pbar:
            ...
            source_metadata = _extract_copyright_metadata(content)
            effective_metadata_overrides = {
                **source_metadata,
            ...
            resolved_title, resolved_author, resolved_language, metadata_payload = _resolve_book_metadata(
                ...
                metadata_overrides=effective_metadata_overrides,
            ...
            with tqdm(total=total_chapters, desc="  Processing chapters", disable=not show_progress,
                ...
                for volume in volumes:
                    if volume.title:  # If has volume title
                        ...
                        volume_page = create_volume_page(
                            volume.title, volume_file_name, len(volume.chapters), watermark,
                            language=resolved_language
                        )
                        ...
                        for chapter in volume.chapters:
                            ...
                            if chapter.sections:  # Chapter has sections
                                ...
                                for section in chapter.sections:
                                    ...
                                    section_page = create_section_page(
                                        section.title, section.content, section_file_name,
                                        language=resolved_language
                                    )
                                ...
                                chapter_page = create_chapter(
                                    chapter.title,
                                    (
                                        metadata_payload.get('copyright_content')
                                        if (chapter.title or '').strip() == '판권'
            ...
            if metadata_payload.get('copyright_page') and not _has_copyright_chapter(volumes):
                copyright_page = create_chapter(
                    "판권",
                    _build_copyright_page_content(resolved_title, resolved_author, metadata_payload),
                    "copyright.xhtml",
                    language=resolved_language
                )
                book.add_item(copyright_page)
                chapter_items.append(copyright_page)
                toc_structure.append(epub.Link("copyright.xhtml", "판권", "copyright"))
            ...
            # Set book structure
            # Add a reader-visible Korean/Chinese/English contents page. The
            # EPUB navigation document remains available for reader menus.
            toc_page = create_toc_page(toc_structure, language=resolved_language)
            book.add_item(toc_page)

            # Reader navigation and linear reading order both begin with the
            # cover and the visible contents page.
            navigation_toc = []
            cover_page = book.get_item_with_id('cover')
            if cover_page is not None:
                navigation_toc.append(epub.Link(cover_page.file_name, '표지', 'cover-link'))
            navigation_toc.append(epub.Link(toc_page.file_name, '목차', 'contents-link'))
            navigation_toc.extend(toc_structure)
            book.toc = navigation_toc
            book.add_item(epub.EpubNcx())
            book.add_item(epub.EpubNav())

            # Add style and set spine
            add_css_style(book)
            book.spine = ([cover_page] if cover_page is not None else []) + [toc_page] + chapter_items
