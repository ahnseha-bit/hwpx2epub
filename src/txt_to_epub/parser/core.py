...
def parse_chapters_from_content(content: str, language: str = 'chinese', config: Optional[ParserConfig] = None, llm_assistant=None, context=None, resume_state=None) -> List[Chapter]:
    ...
    # Apply user-defined chapter patterns. Each custom pattern is treated as a
    # full chapter-title line, so Korean formats like "1화 ..." and "판권"
    # can create independent EPUB chapters.
    if config.custom_chapter_patterns:
        custom_pattern = re.compile(
            r'(?:^|(?<=\n))'
            r'(' + '|'.join(f'(?:{pattern})' for pattern in config.custom_chapter_patterns) + r')'
            r'[ \t]*'
            r'(?=\r?\n|$)',
            re.MULTILINE
        )
        all_matches.extend(custom_pattern.finditer(content))
        all_matches = sorted(all_matches, key=lambda match: match.start())
