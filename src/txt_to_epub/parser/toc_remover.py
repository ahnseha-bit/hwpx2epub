...
def remove_table_of_contents(content: str, language: str = None, llm_assistant=None, config: Optional[ParserConfig] = None) -> str:
    ...
    # Custom chapter patterns must also participate in TOC boundary detection.
    # This is essential for Korean titles such as "1장 제목".
    chapter_patterns.extend(
        re.compile(pattern, re.MULTILINE) for pattern in config.custom_chapter_patterns
    )
    ...
    # Find TOC end
    if toc_start != -1:
        # Korean manuscripts commonly repeat the first TOC entry verbatim when
        # the actual body starts. Remove the index block up to that repetition,
        # while preserving the repeated line as the first real chapter title.
        if language == 'korean':
            toc_entry_idx = next(
                (
                    idx for idx in range(toc_start + 1, len(lines))
                    if lines[idx].strip()
                    and any(pattern.search(lines[idx].strip()) for pattern in chapter_patterns)
                ),
                -1,
            )
            if toc_entry_idx != -1:
                first_entry = lines[toc_entry_idx].strip()
                body_start = next(
                    (
                        idx for idx in range(toc_entry_idx + 1, len(lines))
                        if lines[idx].strip() == first_entry
                    ),
                    -1,
                )
                if body_start != -1:
                    toc_end = body_start - 1
                    # A lone short line before "목차" is normally the book
                    # title already represented by EPUB metadata, not a preface.
                    before_toc = [line.strip() for line in lines[:toc_start] if line.strip()]
                    if len(before_toc) <= 1:
                        toc_start = 0
        ...
        for i in range(toc_start + 1, len(lines)) if toc_end == -1 else ():
